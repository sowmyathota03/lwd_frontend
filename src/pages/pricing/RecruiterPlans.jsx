...
export default function RecruiterPlans() {
  ...
  const fetchPlans = async () => {
    try {
      ...
      console.error("Failed to fetch recruiter plans:", error);
      setPlans([]);
    }
  };
    ...
    <div className="lwd-page min-h-screen bg-slate-50 px-4 py-8 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold sm:text-4xl">Recruiter Plans</h1>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400 sm:text-base">
            Choose a plan to unlock hiring features.
          </p>
        </div>
