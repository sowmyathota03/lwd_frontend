...
export default function RecruiterAdminPlans() {
  ...
  const fetchPlans = async () => {
    try {
      const data = await getRecruiterAdminPlans();
      setPlans(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error("Failed to fetch Company Admin plans:", error);
      setPlans([]);
    }
  };
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400 sm:text-base">
            Choose the best plan for Company Admin features and hiring tools.
          </p>
        </div>
          ...
          {plans.length === 0 ? (
            <div className="py-16 text-center">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                No Company Admin plans available right now.
              </p>
            </div>
          ) : (
